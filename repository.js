...
Repository.prototype.isClosed = function(branch){
    return utils.exist(this.CLOSED_BRANCHES[branch]);
};
...
Repository.prototype.commit = function(rawdata,message,author,branch){
    ...
    if (utils.notExist(this.HEAD[branch])){
    ...
    if (utils.notExist(this.BRANCHES[branch])){
...
Repository.prototype.fork = function(hash, dest){
    ...
    var newBranch = utils.castTo(CommitData,clone(target));
    ...
    while(ptr != null){
        ptr = this.INDEX[ptr];
        var dup = utils.castTo(CommitData,clone(ptr));
...
Repository.prototype.print = function(){
    utils.print("Repositorio: "+this._name);
    utils.print("Branches")
...
Repository.prototype.current_branches = function(){
    ...
    for(var branch in this.BRANCHES){
        if(utils.notExist(this.CLOSED_BRANCHES[branch])){
...
function castRepository(obj){
    var r = utils.castTo(Repository,obj);
    utils.convertAll(CommitData,r.INDEX)
    utils.convertAll(CommitData,r.HEAD)
    utils.convertAll(CommitData,r.BRANCHES)
    return r;
}

function castDataHistory(obj){
    var r = utils.castTo(DataHistory,obj);
    utils.convertAll(CommitData,r.INDEX)
    utils.convertAll(CommitData,r.HEAD)
    utils.convertAll(CommitData,r.BRANCHES)
    return r;
...
CommitData.prototype.print = function(repo){    
    utils.print("Branch: "+this._branch+ "   Message: " + this._message);
    utils.print(this._data);
...
module.exports = {    
    ...
    castDataHistory: castDataHistory
};
